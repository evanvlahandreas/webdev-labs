...
const handleServerMessage = ev => {
    ...
    if (data.type === "login") {
        ...
        for (var i = 0; i < data.active_users.length; i++) {
            document.querySelector('#users-list').innerHTML += `<li>${data.active_users[i]} has joined the chat</li>`
        } 
        ...
        document.querySelector('#users-list').innerHTML = ``;
        for (var i = 0; i < data.active_users.length; i++) {
            document.querySelector('#users-list').innerHTML += `<li>${data.active_users[i]} has left the chat</li>`
        }
        ...
        if (data.username == username) {
            qs('#chat').innerHTML += `<li class="right" aria-live="off"> ${data.username + ": " + data.text}</li>`
        } else {
